/*
Edge Event Schemas

No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)

API version: 1.0.0
*/

// Code generated by OpenAPI Generator (https://openapi-generator.tech); DO NOT EDIT.

package events

import (
	"encoding/json"
	"bytes"
	"fmt"
)

// checks if the AeAccountRemovedAmsDetail type satisfies the MappedNullable interface at compile time
var _ MappedNullable = &AeAccountRemovedAmsDetail{}

// AeAccountRemovedAmsDetail AMS account has been removed
type AeAccountRemovedAmsDetail struct {
	// Source's unique identifier for account
	Id string `json:"id"`
	// Auction Edge unique identifier for an auction.
	AuctionId string `json:"auction-id"`
	Account *CommonAmsAccountPointer `json:"account,omitempty"`
	Initiator *CommonInitiator `json:"initiator,omitempty"`
}

type _AeAccountRemovedAmsDetail AeAccountRemovedAmsDetail

// NewAeAccountRemovedAmsDetail instantiates a new AeAccountRemovedAmsDetail object
// This constructor will assign default values to properties that have it defined,
// and makes sure properties required by API are set, but the set of arguments
// will change when the set of required properties is changed
func NewAeAccountRemovedAmsDetail(id string, auctionId string) *AeAccountRemovedAmsDetail {
	this := AeAccountRemovedAmsDetail{}
	this.Id = id
	this.AuctionId = auctionId
	return &this
}

// NewAeAccountRemovedAmsDetailWithDefaults instantiates a new AeAccountRemovedAmsDetail object
// This constructor will only assign default values to properties that have it defined,
// but it doesn't guarantee that properties required by API are set
func NewAeAccountRemovedAmsDetailWithDefaults() *AeAccountRemovedAmsDetail {
	this := AeAccountRemovedAmsDetail{}
	return &this
}

// GetId returns the Id field value
func (o *AeAccountRemovedAmsDetail) GetId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.Id
}

// GetIdOk returns a tuple with the Id field value
// and a boolean to check if the value has been set.
func (o *AeAccountRemovedAmsDetail) GetIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.Id, true
}

// SetId sets field value
func (o *AeAccountRemovedAmsDetail) SetId(v string) {
	o.Id = v
}

// GetAuctionId returns the AuctionId field value
func (o *AeAccountRemovedAmsDetail) GetAuctionId() string {
	if o == nil {
		var ret string
		return ret
	}

	return o.AuctionId
}

// GetAuctionIdOk returns a tuple with the AuctionId field value
// and a boolean to check if the value has been set.
func (o *AeAccountRemovedAmsDetail) GetAuctionIdOk() (*string, bool) {
	if o == nil {
		return nil, false
	}
	return &o.AuctionId, true
}

// SetAuctionId sets field value
func (o *AeAccountRemovedAmsDetail) SetAuctionId(v string) {
	o.AuctionId = v
}

// GetAccount returns the Account field value if set, zero value otherwise.
func (o *AeAccountRemovedAmsDetail) GetAccount() CommonAmsAccountPointer {
	if o == nil || IsNil(o.Account) {
		var ret CommonAmsAccountPointer
		return ret
	}
	return *o.Account
}

// GetAccountOk returns a tuple with the Account field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AeAccountRemovedAmsDetail) GetAccountOk() (*CommonAmsAccountPointer, bool) {
	if o == nil || IsNil(o.Account) {
		return nil, false
	}
	return o.Account, true
}

// HasAccount returns a boolean if a field has been set.
func (o *AeAccountRemovedAmsDetail) HasAccount() bool {
	if o != nil && !IsNil(o.Account) {
		return true
	}

	return false
}

// SetAccount gets a reference to the given CommonAmsAccountPointer and assigns it to the Account field.
func (o *AeAccountRemovedAmsDetail) SetAccount(v CommonAmsAccountPointer) {
	o.Account = &v
}

// GetInitiator returns the Initiator field value if set, zero value otherwise.
func (o *AeAccountRemovedAmsDetail) GetInitiator() CommonInitiator {
	if o == nil || IsNil(o.Initiator) {
		var ret CommonInitiator
		return ret
	}
	return *o.Initiator
}

// GetInitiatorOk returns a tuple with the Initiator field value if set, nil otherwise
// and a boolean to check if the value has been set.
func (o *AeAccountRemovedAmsDetail) GetInitiatorOk() (*CommonInitiator, bool) {
	if o == nil || IsNil(o.Initiator) {
		return nil, false
	}
	return o.Initiator, true
}

// HasInitiator returns a boolean if a field has been set.
func (o *AeAccountRemovedAmsDetail) HasInitiator() bool {
	if o != nil && !IsNil(o.Initiator) {
		return true
	}

	return false
}

// SetInitiator gets a reference to the given CommonInitiator and assigns it to the Initiator field.
func (o *AeAccountRemovedAmsDetail) SetInitiator(v CommonInitiator) {
	o.Initiator = &v
}

func (o AeAccountRemovedAmsDetail) MarshalJSON() ([]byte, error) {
	toSerialize,err := o.ToMap()
	if err != nil {
		return []byte{}, err
	}
	return json.Marshal(toSerialize)
}

func (o AeAccountRemovedAmsDetail) ToMap() (map[string]interface{}, error) {
	toSerialize := map[string]interface{}{}
	toSerialize["id"] = o.Id
	toSerialize["auction-id"] = o.AuctionId
	if !IsNil(o.Account) {
		toSerialize["account"] = o.Account
	}
	if !IsNil(o.Initiator) {
		toSerialize["initiator"] = o.Initiator
	}
	return toSerialize, nil
}

func (o *AeAccountRemovedAmsDetail) UnmarshalJSON(data []byte) (err error) {
	// This validates that all required properties are included in the JSON object
	// by unmarshalling the object into a generic map with string keys and checking
	// that every required field exists as a key in the generic map.
	requiredProperties := []string{
		"id",
		"auction-id",
	}

	allProperties := make(map[string]interface{})

	err = json.Unmarshal(data, &allProperties)

	if err != nil {
		return err;
	}

	for _, requiredProperty := range(requiredProperties) {
		if _, exists := allProperties[requiredProperty]; !exists {
			return fmt.Errorf("no value given for required property %v", requiredProperty)
		}
	}

	varAeAccountRemovedAmsDetail := _AeAccountRemovedAmsDetail{}

	decoder := json.NewDecoder(bytes.NewReader(data))
	decoder.DisallowUnknownFields()
	err = decoder.Decode(&varAeAccountRemovedAmsDetail)

	if err != nil {
		return err
	}

	*o = AeAccountRemovedAmsDetail(varAeAccountRemovedAmsDetail)

	return err
}

type NullableAeAccountRemovedAmsDetail struct {
	value *AeAccountRemovedAmsDetail
	isSet bool
}

func (v NullableAeAccountRemovedAmsDetail) Get() *AeAccountRemovedAmsDetail {
	return v.value
}

func (v *NullableAeAccountRemovedAmsDetail) Set(val *AeAccountRemovedAmsDetail) {
	v.value = val
	v.isSet = true
}

func (v NullableAeAccountRemovedAmsDetail) IsSet() bool {
	return v.isSet
}

func (v *NullableAeAccountRemovedAmsDetail) Unset() {
	v.value = nil
	v.isSet = false
}

func NewNullableAeAccountRemovedAmsDetail(val *AeAccountRemovedAmsDetail) *NullableAeAccountRemovedAmsDetail {
	return &NullableAeAccountRemovedAmsDetail{value: val, isSet: true}
}

func (v NullableAeAccountRemovedAmsDetail) MarshalJSON() ([]byte, error) {
	return json.Marshal(v.value)
}

func (v *NullableAeAccountRemovedAmsDetail) UnmarshalJSON(src []byte) error {
	v.isSet = true
	return json.Unmarshal(src, &v.value)
}


